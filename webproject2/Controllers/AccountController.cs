...
using AirlinesReservationWebProject2.Models;
    ...
    public class AccountController : Controller
    {
        ...
        public IActionResult Login(LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    if (user.Locked)
                    {
                        ModelState.AddModelError(nameof(model.Username), "Kullanici kilitli");
                        return View(model);
                    }
                    ...
                    ClaimsIdentity idendity = new ClaimsIdentity(claims,
                ...
                {
                    ModelState.AddModelError("", "Kullanici adi yada Parola Hatali");
                }
            }
            return View(model);
        }
        ...
        public IActionResult Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError(nameof(model.Username), "Kullanici adi kullaniliyor");
                    View(model);
                }
                string hashedPassword = DoMD5HashedString(model.Password);
                User user = new()
        ...
        public IActionResult ProfileChangeFullName([Required][StringLength(50)] string? fullname)
        {
            ...
            if (ModelState.IsValid)
            {
                Guid userid = new Guid(User.FindFirstValue(ClaimTypes.NameIdentifier));
                User user = _databaseContext.Users.FirstOrDefault(x => x.Id == userid);
                user.FullName = fullname.ToUpper();
                User.Identity.Name = user.FullName;
